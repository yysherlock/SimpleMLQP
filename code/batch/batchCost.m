function [cost,grad] = batchCost(theta,lambda,inputSize,outputSize,hiddenSize,traindata)
...
U2grad = U2grad/m + lambda*U2;U1grad = U1grad/m + lambda*U1;
V2grad = V2grad/m + lambda*V2;V1grad = V1grad/m + lambda*V1;
b1grad = b1grad/m;b2grad = b2grad/m;

% LMS cost
% cost = 0.5 * (1/m) * (sum(error.^2)) + 0.5*lambda*(sum(sum(U1.*U1))+sum(sum(U2.*U2))+sum(sum(V1.*V1))+sum(sum(V2.*V2)));

% logistic cost
cost = -(1/m) * sum(label.*log(A2) + (1-label).*(log(1-A2)))+ 0.5*lambda*(sum(sum(U1.*U1))+sum(sum(U2.*U2))+sum(sum(V1.*V1))+sum(sum(V2.*V2)));
